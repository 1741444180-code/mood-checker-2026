...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dataType === 'checkins' || dataType === 'all') {
      ...
      response.checkIns = checkIns.map((checkIn) => ({
        id: checkIn.id,
    ...
    // 检查是否有数据
    if (
      (!response.checkIns || response.checkIns.length === 0) &&
    ...
    // 创建 JSON 响应
    const filename = `export_${dataType}_${new Date().toISOString().split('T')[0]}.json`
